...
Variables follow snake case naming style (unless they overlap with
`Shiny` syntax). Try to give meaningful and self explanatory names.
...
- Functions that define active elements should contain `input` in their
  name, eg. `checkbox_input`.

## Overriding shiny functions

When overriding shiny functions we usually follow `shiny` styling.
Usually it requires creating a function with `shiny.semantic` syntax and
then implementing a wrapper that follows `shiny` syntax.
