...
- default:
  ...
  (required) Template for desktop: list(areas = \[data.frame of
  character\], rows_height = \[vector of character\], cols_width =
  \[vector of character\])

- mobile:

  (optional) Template for mobile (screen width below 768px): list(areas
  = \[data.frame of character\], rows_height = \[vector of character\],
...
template - contains template that can be parsed by htmlTemplate()
function
