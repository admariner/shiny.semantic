...
input. Search field is already initialized and available under
input\[\[input_id\]\]. Search will automatically route to the named API
endpoint provided as parameter. API response is expected to be a JSON
with property fields \`title\` and \`description\`. See
https://semantic-ui.com/modules/search.html#behaviors for more details.
...
- search_api_url:
  ...
  Register custom API url with server JSON Response containing fields
